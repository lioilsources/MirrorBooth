...
def rag_retriever_node(state: ShaderGenState) -> ShaderGenState:
    ...
    try:
        # imported lazily so the pipeline stays usable when chromadb / the
        # embedding model are unavailable (offline) — RAG just yields no context.
        import chromadb
        ...
        # RAG DB not built / chromadb missing — proceed without context
        snippets = []
